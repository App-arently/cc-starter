...
This skill requires a headless browser tool. Options:

1. **Claude-in-Chrome MCP** — If `mcp__claude-in-chrome__navigate` is available, use the MCP browser tools directly.
2. **Playwright** — `npx playwright install && npx playwright` or use Playwright MCP server.
3. **Puppeteer** — `npx puppeteer` or configure as MCP server.
4. **Custom** — Set `BROWSE_BIN` env var to your headless browser binary.

The rest of this skill uses generic browser commands. Adapt to your tool:
- **Navigate:** `goto <url>` or `navigate <url>`
- **Screenshot:** `screenshot <path>`
- **Click:** `click <selector>`
- **Fill form:** `fill <selector> <value>`
- **Read page:** `get_page_text` or `snapshot`
- **Console errors:** `read_console_messages` or `console --errors`
...
1. Verify browser tool is available (see Setup above)
...
2. Fill in credentials (NEVER include real passwords in report — use `[REDACTED]`)
3. Submit and verify login succeeded
...
**For SPAs:** Navigation links may be sparse because routing is client-side. Look for nav elements (buttons, menu items) in the page content instead.
...
- Look for nav elements in page content — link discovery misses client-side routes
