...
<!-- Customize test commands for your project. See Step 3 for auto-detection. -->
...
Auto-detect and run the project's test suite. Check for test commands in this order:

1. **package.json** — look for `test`, `test:unit`, `test:all` scripts → `npm test` or `npm run test:all`
2. **Makefile** — look for `test` target → `make test`
3. **pytest** — check for `pytest.ini`, `pyproject.toml [tool.pytest]`, or `tests/` dir → `pytest`
4. **Go** — check for `go.mod` → `go test ./...`
5. **Cargo** — check for `Cargo.toml` → `cargo test`
6. **Mix** — check for `mix.exs` → `mix test`

Run all detected test suites. If multiple (e.g., backend + frontend), run them in parallel:

```bash
<test-command-1> 2>&1 | tee /tmp/ship_tests_1.txt &
<test-command-2> 2>&1 | tee /tmp/ship_tests_2.txt &
wait
```
...
1. Read the current `VERSION` file (4-digit format: `MAJOR.MINOR.PATCH.MICRO`). If no VERSION file exists, create one with `0.1.0.0`.
...
> **Note:** This uses a 4-digit version format. Adapt to your project's convention (semver 3-digit, calver, etc.) as needed.

---

## Step 5: CHANGELOG (auto-generate)

1. Read `CHANGELOG.md` header to know the format. If no CHANGELOG exists, create one with a standard header.
...
2. **Commit ordering** (earlier commits first):
   ...
   - **Core logic:** new models, services, utilities (with their tests)
   - **Interface:** controllers, views, components (with their tests)
...
- **Always use the 4-digit version format** from the VERSION file (adapt if your project uses a different format).
